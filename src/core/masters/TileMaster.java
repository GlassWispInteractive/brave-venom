						...
						if (Files.isRegularFile(filePath)) {
							...
							if (xml.exists()) {
								...
								try {
									...
									parser.parse(xml.toURI().toString());
									...
									NodeList tiles = main.getChildNodes();
									
									for (int i = 0; i < tiles.getLength(); i++) {
										Node node = tiles.item(i);
										System.out.println(node.getAttributes().getNamedItem("height"));
										

//										for (int j = 0; j < attr.getLength(); j++) {
//											System.out.println(attr.getNamedItem("height"));
//										}
									}

									// Navigate down the hierarchy to get to the
									// CEO node
									// Node comp = getNode("Company", root);
									// Node exec = getNode("Executive",
									// comp.getChildNodes() );
									// String execType = getNodeAttr("type",
									// exec);

									// Load the executive's data from the XML
									// NodeList nodes = exec.getChildNodes();
									// String lastName =
									// getNodeValue("LastName", nodes);
									// String firstName =
									// getNodeValue("FirstName", nodes);
									// String street = getNodeValue("street",
									// nodes);
									// String city = getNodeValue("city",
									// nodes);
									// String state = getNodeValue("state",
									// nodes);
